...
class Task(models.Model):
    ...
    def run_export(self):
        self.ensure_one()
        config_obj = self.env['external.file.task.config']
        attachment_obj = self.env['ir.attachment']
        configs = config_obj.search([('task_id','=',self.id)])
        for config in configs:
            _logger.info('Running task config with id %s', config.id)
            attachments = attachment_obj.search([('state', '!=', 'done'), ('res_field','=', config.field_id.name), \
                ('res_model','=', config.model_id.model)])
            for attachment in attachments:
                _logger.info('Running attachment with id %s', attachment.id)
                res_model = self.env[attachment.res_model].search(expression.normalize_domain(safe_eval(config.domain)))
                if not res_model or attachment.res_id not in res_model.ids:
                    _logger.info('Skipped execution on attachment with id %s and domain %s' %(attachment.id, config.domain))
                    continue
                attachment.write({'file_type':'export_external_location'})
