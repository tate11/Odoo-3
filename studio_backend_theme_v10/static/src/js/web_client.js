...
var AppChanger = require('studio_backend_theme_v10.AppChanger');
var Menu = require('studio_backend_theme_v10.Menu');
...
return AbstractWebClient.extend({
    custom_events: _.extend({}, AbstractWebClient.prototype.custom_events, {
        ...
        scrollTo: 'scrollTo',
        show_app_changer: function () {
            this.toggle_app_changer(true);
        },
        hide_app_changer: function () {
            // Restore the url
            $.bbq.pushState(this.url, 2); // merge_mode 2 to replace the current state
            this.toggle_app_changer(false);
        },
    }),
    start: function () {
        var self = this;
        ...
        return this._super.apply(this, arguments).then(function () {
            // Listen to 'scroll' event in app_changer and propagate it on main bus
            self.app_changer.$el.on('scroll', core.bus.trigger.bind(core.bus, 'scroll'));
        });
    },
    ...
    instanciate_menu_widgets: function() {
        ...
        return this.load_menus().then(function(menu_data) {
            ...
            // into the DOM.
            if (self.app_changer) {
                self.app_changer.destroy();
            }
            if (self.menu) {
                self.menu.destroy();
            }
            self.app_changer = new AppChanger(self, menu_data);
            self.menu = new Menu(self, menu_data);

            defs.push(self.app_changer.appendTo(document.createDocumentFragment()));
    ...
    // --------------------------------------------------------------
    /**
     * Extends do_action() to toggle the appchanger off if the action isn't displayed in a dialog
     */
    do_action: function () {
        var self = this;
        return this._super.apply(this, arguments).then(function(action) {
            if (self.menu.appchanger_displayed && action.target !== 'new') {
                self.toggle_app_changer(false);
            }
        });
    },
            ...
            if (!_.isEqual(self._current_state, stringstate)) {
                ...
                if (state.action || (state.model && (state.view_type || state.id))) {
                    ...
                    return self.action_manager.do_load_state(state, !!self._current_state).then(function () {
                        ...
                        self.toggle_app_changer(false);
                    }).fail(self.toggle_app_changer.bind(self, true));
    ...
    toggle_app_changer: function (display) {
        if (display === this.app_changer_displayed) {
        ...
        if (display) {
            ...
            this.clear_uncommitted_changes().then(function() {
                ...
                // Attach the app_changer
                self.append_app_changer();
            ...
            framework.detach([{widget: this.app_changer}]);
            framework.append(this.$el, [this.web_client_content], {
                in_DOM: true,
                callbacks: [{widget: this.action_manager}],
            });
            this.app_changer_displayed = false;
            this.menu.toggle_mode(false, this.action_manager.get_inner_action() !== null);
        }
    },
    append_app_changer: function () {
        framework.append(this.$el, [this.app_changer.$el], {
            in_DOM: true,
            callbacks: [{widget: this.app_changer}],
        });
        this.app_changer_displayed = true;
    },
    // --------------------------------------------------------------
    // Scrolltop handling
    // --------------------------------------------------------------
