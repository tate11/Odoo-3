    ...
    def onchange_action(self):
        if self.action_id:
            ...
            if self.action_id.expected_type == 't':
                delay = self.action_id.expected_duration
                expected_date = datetime.today()
            if self.action_id.expected_type == 'd' and self.action_id.expected_date_field_id:
                e_date = getattr(self.task_id, str(self.action_id.expected_date_field_id.name))
                delay = self.action_id.delay_expected_date_field_id
                if e_date:
                    expected_date = datetime.strptime(e_date.split(' ')[0],DT)
                else: 
                    expected_date = datetime.today()
            if self.action_id.expected_duration_unit == 'd':
                days = delay
            if self.action_id.expected_duration_unit == 'bd':
                days = ((expected_date + BDay(delay)) - expected_date).days + 1
            ...
            if self.action_id.expected_duration_unit == 'y':
                years = delay
            if expected_date:
                self.expected_date = expected_date + relativedelta(years=years, months=months, weeks=weeks, days=days)
            else:
        ...
        """if self.action_id.expected_type == 't':
            if self.action_id:
                ...
                if self.action_id.expected_duration_unit == 'bd':
                    # days between n business days and today
            ...
            if self.action_id.expected_date_field_id:
                ...
                if expected_date:
                    expected_date = datetime.strptime(expected_date.split(' ')[0],DT)
                else:
                    expected_date = datetime.today()
            delay = 0
